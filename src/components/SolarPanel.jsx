            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">{inv.id} • {inv.firmware}</p>
                </div>
              </div>
              <span className="bg-battery-green/15 text-battery-green text-xs px-2.5 py-1 rounded-full font-medium">
                {inv.status}
              </span>
            </div>
          ...
          {eg4Data.batteries.map((bat, i) => (
            <div key={bat.id} className="bg-slate-900/50 rounded-lg p-4 border border-slate-700/30">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs text-slate-400">{bat.cycles} cycles</span>
              </div>
              {/* SOC Bar */}
              <div className="w-full bg-slate-700 rounded-full h-3 mb-3">
                <div
                  className="h-3 rounded-full transition-all"
                  style={{ width: `${bat.soc}%`, backgroundColor: batteryColors[i] }}
                />
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  ...
                  <p className="text-battery-green font-semibold">{bat.health}%</p>
                </div>
              </div>
            </div>
          ))}
